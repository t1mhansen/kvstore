...
//
// This is the happy-path replay: it assumes every record in the file is
// intact. Handling a truncated/corrupt tail from a real crash is its own
// problem, saved for later.
void StorageEngine::RebuildIndexFromLog() {
  ...
  while (offset < file_size) {
    ...
    const DecodedRecord record = DecodeLogRecord(buffer.data(), record_len);
    if (record.tombstone) {
